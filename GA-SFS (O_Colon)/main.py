...
def evaluate_metrics(X, y, classifier, n_splits=5):
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        if hasattr(clf, "predict_proba"):
            ...
            aucs.append(roc_auc_score(y_test, y_probs, multi_class="ovr", average="weighted"))
        else:
...
# Feature selection using SVM with SMOTEENN
selected_features, svm_metrics_during_selection = feature_selection_hybrid(X, y, m_ratio=0.2, n_ratio=0.1, n_splits=5)

# Final evaluation of the selected feature subset using both SVM and KNN with SMOTEENN
