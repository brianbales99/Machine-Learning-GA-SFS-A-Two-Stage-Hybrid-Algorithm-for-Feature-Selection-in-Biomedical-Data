...
# Load Breast Cancer Wisconsin (Prognostic) Dataset
def load_data():
    url = "https://archive.ics.uci.edu/ml/machine-learning-databases/breast-cancer-wisconsin/wdbc.data"
    df = pd.read_csv(url, header=None)

    # Features and target selection
    X = df.iloc[:, 2:].values  # Features start from column 2 onward
    y = df.iloc[:, 1].values  # Target variable is in column 1

    # Encode target: 'M' as 1 and 'B' as 0
    y = np.where(y == 'M', 1, 0)
    return X, y
    ...
    def adaptive_mutation(offspring, scores, base_mutation_rate):
        ...
        for idx, chromosome in enumerate(offspring):
            adaptive_rate = base_mutation_rate * (
                        1 - (scores[idx] / max_score)) if max_score > 0 else base_mutation_rate
