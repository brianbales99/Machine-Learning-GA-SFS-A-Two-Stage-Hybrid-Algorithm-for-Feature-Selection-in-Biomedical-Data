...
from sklearn.neighbors import KNeighborsClassifier  # Add KNN
from imblearn.combine import SMOTEENN  # Add SMOTEENN for combined sampling
from ucimlrepo import fetch_ucirepo

# Fetch SPECTF Heart Dataset using ucimlrepo
def load_data():
    spectf_heart = fetch_ucirepo(id=96)
    X = spectf_heart.data.features
    y = spectf_heart.data.targets.values.ravel()  # Flatten y to a 1D array
    return X.values, y
...
# Apply combined resampling with SMOTEENN
def apply_combined_sampling(X, y):
    smote_enn = SMOTEENN()  # Initialize SMOTEENN
    X_resampled, y_resampled = smote_enn.fit_resample(X, y)
    return X_resampled, y_resampled

# Main Genetic Algorithm with helper methods embedded
...
# Apply combined sampling with SMOTEENN
...
best_chromosome, best_score = genetic_algorithm(X_resampled, y_resampled, num_generations, population_size, num_parents_mating,
