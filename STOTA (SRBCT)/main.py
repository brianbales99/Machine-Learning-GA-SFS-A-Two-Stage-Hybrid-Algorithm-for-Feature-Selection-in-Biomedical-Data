...
from imblearn.combine import SMOTEENN  # Import SMOTEENN for handling imbalanced datasets

# Load dataset for multiclass classification
def load_data():
    filename = 'O_SRBCT(in).csv'  # Your multiclass dataset
...
# Apply SMOTEENN for combined sampling (oversampling + undersampling)
def apply_combined_sampling(X, y):
    smote_enn = SMOTEENN()  # Initialize SMOTEENN
    X_resampled, y_resampled = smote_enn.fit_resample(X, y)  # Resample dataset
...
def genetic_algorithm(X, y, num_generations, population_size, num_parents_mating, base_mutation_rate, n_splits=5):
    ...
    def evaluate_auc_svm(X_selected, y):
        ...
        for train_index, test_index in skf.split(X_selected, y):
            ...
            # AUC may not always apply to multiclass unless a binarized approach is taken
            aucs.append(roc_auc_score(y_test, y_probs, multi_class='ovr'))  # Using one-vs-rest for AUC
        return np.mean(aucs)
...
# Apply SMOTEENN combined sampling to handle imbalanced data
X_resampled, y_resampled = apply_combined_sampling(X, y)
...
best_chromosome, best_score = genetic_algorithm(X_resampled, y_resampled, num_generations, population_size,
                                                num_parents_mating, base_mutation_rate, n_splits=n_splits)
...
X_test_selected = X_resampled[:, best_chromosome == 1]
svm_final = SVC(probability=True)
svm_final.fit(X_test_selected, y_resampled)
test_predictions_svm = svm_final.predict(X_test_selected)

final_accuracy_svm = accuracy_score(y_resampled, test_predictions_svm)
final_balanced_accuracy_svm = balanced_accuracy_score(y_resampled, test_predictions_svm)
final_f1_svm = f1_score(y_resampled, test_predictions_svm, average='weighted')
final_precision_svm = precision_score(y_resampled, test_predictions_svm, average='weighted')
final_recall_svm = recall_score(y_resampled, test_predictions_svm, average='weighted')
...
knn_final.fit(X_test_selected, y_resampled)
test_predictions_knn = knn_final.predict(X_test_selected)

final_accuracy_knn = accuracy_score(y_resampled, test_predictions_knn)
final_balanced_accuracy_knn = balanced_accuracy_score(y_resampled, test_predictions_knn)
final_f1_knn = f1_score(y_resampled, test_predictions_knn, average='weighted')
final_precision_knn = precision_score(y_resampled, test_predictions_knn, average='weighted')
final_recall_knn = recall_score(y_resampled, test_predictions_knn, average='weighted')
