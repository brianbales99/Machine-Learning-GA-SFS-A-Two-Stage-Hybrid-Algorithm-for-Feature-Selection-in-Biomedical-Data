...
from sklearn.preprocessing import StandardScaler, LabelEncoder  # Add LabelEncoder for encoding words to binary labels
from sklearn.metrics import accuracy_score, balanced_accuracy_score, f1_score, precision_score, recall_score, \
    roc_auc_score
from sklearn.svm import SVC
from sklearn.neighbors import KNeighborsClassifier

# Load dataset for binary classification (with word labels)
def load_data():
    filename = 'O_Lung(in).csv'  # Your binary dataset with words as labels
    ...
    y = array[:, -1]  # Target variable (last column with word labels)

    # Label encode the word labels to binary labels
    le = LabelEncoder()
    y_encoded = le.fit_transform(y)  # Convert word labels to 0 and 1
    return X, y_encoded
...
def genetic_algorithm(X, y, num_generations, population_size, num_parents_mating, base_mutation_rate, n_splits=5):
    ...
    def evaluate_population_with_f1(population):
        ...
        for chromosome in population:
            ...
            for train_index, val_index in skf.split(X_selected, y):
                ...
                # Use weighted average for binary F1 score
    ...
    def evaluate_auc_svm(X_selected, y):
        ...
        for train_index, test_index in skf.split(X_selected, y):
            ...
            aucs.append(roc_auc_score(y_test, y_probs[:, 1]))  # For binary classification
        return np.mean(aucs)
