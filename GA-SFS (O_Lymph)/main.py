...
# Load dataset
def load_data():
    filename = 'O_Lymph(in).csv'  # Update for multiclass dataset
...
def evaluate_metrics(X, y, classifier, n_splits=5):
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        if hasattr(clf, "predict_proba"):
            ...
            aucs.append(roc_auc_score(y_test, y_probs, multi_class="ovr"))
        else:
            aucs.append(float("nan"))

        accuracies.append(accuracy_score(y_test, y_pred))
        balanced_accuracies.append(balanced_accuracy_score(y_test, y_pred))
        recalls.append(recall_score(y_test, y_pred, average='weighted', zero_division=1))  # Updated with zero_division
        precisions.append(precision_score(y_test, y_pred, average='weighted', zero_division=1))  # Updated
        f1_scores.append(f1_score(y_test, y_pred, average='weighted', zero_division=1))  # Updated
...
def feature_selection_hybrid(X, y, m_ratio=0.2, n_ratio=0.1, n_splits=5):
    ...
    while best_auc <= 0:
        ...
        metrics = evaluate_metrics(X_initial, y, SVC(probability=True, random_state=42, decision_function_shape='ovr'), n_splits)
        initial_auc = metrics['AUC']

        if initial_auc > evaluate_metrics(X, y, SVC(probability=True, random_state=42, decision_function_shape='ovr'), n_splits)['AUC']:
    ...
    while True:
        ...
        while candidates:
            ...
            for candidate in candidates:
                ...
                candidate_metrics.append(evaluate_metrics(X_temp, y, SVC(probability=True, random_state=42, decision_function_shape='ovr'), n_splits))
        ...
        while len(selected_features) > m:
            ...
            for feature in selected_features:
                ...
                removal_metrics.append(evaluate_metrics(X_temp, y, SVC(probability=True, random_state=42, decision_function_shape='ovr'), n_splits))
...
svm_final_metrics = evaluate_metrics(X_selected, y, SVC(probability=True, random_state=42, decision_function_shape='ovr'), n_splits=5)
